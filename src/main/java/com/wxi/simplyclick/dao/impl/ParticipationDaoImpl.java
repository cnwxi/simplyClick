...
//        String sql = "replace into participation values(?,?,?)";
        String sql = "update  Participation set role=? where filmId=? and castId=? and `character`= ? ";
        Object[] objects = {participation.getRole(), participation.getFilmId(), participation.getCastId(), participation.getCharacter()};
        int update = jdbcTemplate.update(sql, objects);
        return update > 0;
    }
}
